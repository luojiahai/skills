#!/usr/bin/env bash
#
# archive.sh — the Douyin platform run, reached through the skill dispatcher.
#
...
STATE_DIR="${XDG_STATE_HOME:-$HOME/.local/state}/archiver/douyin"
...
# needs one to exist.
PLAN_HINT="${ARCHIVE_SELF:-${SCRIPT_DIR}/archive.sh} '${URL}'${ARCHIVES_ARG:+ --archives '${ARCHIVES_ARG}'}${ALIAS:+ --alias '${ALIAS}'} --plan"
...
if ! command -v yt-dlp >/dev/null 2>&1; then
  ...
  echo "Or run ${SKILL_DIR}/setup.sh douyin, which checks everything at once." >&2
  exit 1
fi
...
if [[ ! -d "${STATE_DIR}/node_modules/playwright" && ! -d "${SKILL_DIR}/node_modules/playwright" ]]; then
  echo "error: playwright not installed — run: ${SKILL_DIR}/setup.sh douyin" >&2
  exit 1
fi
