...
const accountDir = async () => {
  const dir = path.join(await mkdtemp(path.join(os.tmpdir(), 'douyin-sync-')), 'douyin', 'MS4wSEC');
...
test('a plan survives a round trip', async () => {
  ...
  await savePlan(dir, { createdAt: 'now', posts: [{ postId: '1' }] });
  assert.deepEqual((await loadPlan(dir)).posts, [{ postId: '1' }]);
});
...
test('a file from a version this build cannot read is no plan at all', async () => {
  ...
  await writeFile(
    ...
    JSON.stringify({ version: SYNC_VERSION + 1, plan: { posts: [{ postId: '1' }] } }),
  );
  assert.equal(await readSync(dir), null);
  assert.equal(await loadPlan(dir), null);
});
...
test('writeSync creates the account folder if the run got here first', async () => {
  const dir = path.join(await mkdtemp(path.join(os.tmpdir(), 'douyin-sync-')), 'douyin', 'MS4wNEW');
