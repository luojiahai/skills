...
**Verified against the 2026 income year.** The stamp is the date the section list at step 2 was checked, item by item, against the ATO's published myTax instructions for that year — the section index at `.../mytax-instructions/<year>/`, which is the authority the list is drawn from. It dates that list and nothing else. Rates and thresholds do not go stale, because every one is fetched on this run, and dead ATO URLs self-heal through the search fallback below. It is one literal, deliberately: bump it when the section list is re-checked, and nothing else needs touching.
...
ATO deep URLs are reorganised often and 404 silently. When a URL fails, find the current one with `WebSearch` scoped to `allowed_domains: ["ato.gov.au"]`, then curl it. [figures.md](process/figures.md) holds the stable entry points and the rates to pull from each.
...
The worksheet is the saved state — one file that is both the deliverable and the resume point, so there is no second place for progress to live and go stale. It is updated as each step lands rather than written up at the end.
...
tax-2026/
  sam/
    ...
    bundle/                   the documents, filed by what they feed
      income/
        index.md
        prefill-2026-08-14.pdf
        income-statement-acme.pdf
      deductions/
        index.md
        2026-03-14-officeworks.jpg
...
Find a return in progress by globbing `tax-*/*/worksheet.md`. `joint/` holds `shared.md` rather than a worksheet, so it is never mistaken for a return of its own.

Where the year's folder lands is the user's call and not an assumption: step 1 names the exact path and confirms it before creating anything, saying what will be written there — copies of the income statement, the bank and dividend statements, the receipts and the private health statement, alongside the worksheet's figures. The working directory is wherever they happened to start the session, which is often a code project and sometimes one with a remote.
...
**Found the one** — read the worksheet in full, and `../joint/shared.md` too where it exists, because that file is saved state as much as the worksheet is. Report back before doing any work: what is settled, what the Outstanding register is waiting on, what is sitting in `inbox/`, and which step comes next. Resume at that step. Settled sections are re-opened only if the user asks or a newly arrived document contradicts them.

**Found none for this person and year** — before creating anything, say what this skill is and is not, from *What this is, and what it is not* above: three or four sentences, plain English, no section numbers. Then name the exact path you are about to write — the tree in *Saving and resuming* above, rooted at `tax-<YYYY>/` in the working directory — and confirm it, saying what will be written there. On confirmation, create the year root and the person's folder, copy [WORKSHEET.md](templates/WORKSHEET.md) to `worksheet.md`, create an empty `inbox/`, and add the ignore line if this is a git working tree.
...
myTax's own "personalise" screen groups these differently — it bundles some, selects others for you from prefill, and displays a few unconditionally. Do not try to make the two lists correspond one to one. This list is the exhaustive one; matching it to the screen is step 8's job, not this step's.
...
**Reach.** Batch 0 named the places the records live. For each place, say which tool you will use before you use it, then read what is inside that place — the folder, the page, the database — and take the figures from there. Nothing outside the places the user named.

Reaching a place is one attempt with the tools this session already has. Where there is no tool for a place, or reaching it would mean installing or authorising something, say so in one line and ask the user to export its contents into `inbox/`; read that folder instead.
...
**Read back what you read.** A **material** figure — one that lands on a myTax label: gross payments, tax withheld, interest, franked and unfranked dividends, rent — is read back to the user field by field, per document, before it is written. A **pile** — a stack of receipts, a broker CSV, a logbook — is read back in aggregate: the count, the total, and every line you could not read. Forty confirmations in a row is where a user starts saying yes without looking.
...
**Done when** every document step 2 named is either a copy in `bundle/<section>/`, listed in that section's `index.md` with the figures read off it — or an Outstanding entry naming the document, who it comes from, and **the place that was looked in**; every material figure has been read back to the user field by field and confirmed, and every pile read back as a count and a total; every field that could not be read has been read out by the user or reads `TBC` against an Outstanding entry; `inbox/` is empty; and the worksheet's Bundle table has a row for every section folder on disk.
...
A corrected line is the standard case, not a failure: the worksheet's figure is the authority for what goes into myTax, and the index's figure is what was read off the page. The difference between them is the working.
...
**For a couple**, the ownership splits in `joint/shared.md` — joint account interest, jointly owned property — are settled before this step's labels are worked, because a split settled afterwards means correcting a label that has already gone in.
...
- **For a couple**, the readiness gate in `joint/shared.md` passes: both taxable incomes settled, both registers empty, both sets of shares reconciled against it. Neither return is handed over until it does — one spouse lodging on the other's estimated income is how a couple earns an amendment.
...
- **The bundle is complete** — every document the worksheet cites is a copy under `bundle/`, listed in its section's `index.md` — and it is somewhere that will still exist in five years. A working directory inside a code project is not durable by default; ask where it should live for the long run and say so plainly.
...
**Done when** every check above passes or is explicitly waived by the user, and the user has the worksheet and the bundle in hand.
