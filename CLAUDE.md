...
A skill folder is named for the work it does — either a **gerund phrase naming the activity** (`preparing-tax-return`) or an **agent noun naming what the skill is** (`douyin-archiver`) where it is a lasting wrapper over external tooling — never a bare verb (`prepare-…`). Scope qualifiers — jurisdiction, product, platform — live in the **description** rather than the name, unless the scope *is* the skill's identity and the name reads as generic without it (`douyin-archiver`, not `archiver`); the description is human-facing and appears beside the name wherever the skill is listed, so that is where a reader meets the scope. A skill whose name is generic owes its scope a stated line in `SKILL.md` and in the top-level `README.md`, and a sentence the agent says out loud on the first run.
